...
function print256ColorSample(
    string $heading,
    string $method,
    StyleInterface $additionalStyle = Style\Text::None,
): void {
    global $style;
    ...
    for ($i = 0; $i < 256; ++$i) {
        ...
        echo $style->apply(
            ...
            new Style\Composite($additionalStyle, Style\Color256::{$method}($i)),
        );
    ...
    echo $style->apply(\PHP_EOL, Style\Text::None);
    $style->autoTerminate();
}

function printTrueColorSample(
    string $heading,
    string $method,
    StyleInterface $additionalStyle = Style\Text::None,
): void {
...
print256ColorSample('Underline', 'underline', Style\Text::Underline);
printTrueColorSample('Foreground', 'foreground');
printTrueColorSample('Background', 'background');
printTrueColorSample('Underline', 'underline', Style\Text::Underline);
